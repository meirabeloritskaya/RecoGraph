...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('file_path', type=str, help='Path to the JSON file with users')
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for user_data in users:
                    ...
                    if not email or not password:
                        self.stdout.write(self.style.WARNING(f"Skipping user with incomplete data: {user_data}"))
                        continue
